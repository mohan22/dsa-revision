# Problem: Subsequence DP pattern

## Source
- Platform: Anki deck seed
- Topic: Dynamic Programming
- Tags: DP, Subsequence, LCS, Day18
...
- Pattern hint from tags: DP / Subsequence
- Use this note to reconstruct the full solution before promoting it to Tier 1.

## Core Insight

If s1[i]==s2[j]: dp[i][j]=1+dp[i-1][j-1]. Else dp[i][j]=max(dp[i-1][j], dp[i][j-1]). Used for LCS, LPS, min edits.
