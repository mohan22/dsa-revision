...
- Topic: Trees / BST
- Tags: Tree, DFS, GlobalState, Day12
...
When returning a value upward is not enough (diameter, max path sum). Pattern: global best dfs(node): left=dfs(left); right=dfs(right); best=max(best, compute(left,right,node)); return contribution
