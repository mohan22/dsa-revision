# Problem: Deque-Based Pattern For Sliding Window Max
...
Given an integer array and a sliding window size `k`, the objective is to compute the maximum value in every window of length `k` as the window moves from left to right across the array. The deque-based pattern maintains candidate indices in a way that keeps the current maximum at the front, allowing each window's maximum to be found in amortized O(1) time per element.
...
A brute-force solution checks all `k` values for each window, resulting in O(nk) time. The optimized deque pattern avoids repeated scans by preserving only useful candidates, reducing work to O(n).
