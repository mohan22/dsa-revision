# Problem: What are the main graph representations and when to use them

## Source
- Platform: Anki deck seed
- Topic: Graph / Matrix
- Tags: Graph, Representation, Day20
...
- Pattern hint from tags: Graph / Representation
- Use this note to reconstruct the full solution before promoting it to Tier 1.

## Core Insight

Use adjacency list for sparse graphs; adjacency matrix for dense graphs. Adj list template: graph={node:[] for node in nodes}; for u,v in edges: graph[u].append(v); graph[v].append(u) if undirected.
