# Problem: DP pattern for knapsack problems

## Source
- Platform: Anki deck seed
- Topic: Dynamic Programming
- Tags: DP, Knapsack, Day17
...
0/1 Knapsack: for each item: for w from capacity→item.weight: dp[w]=max(dp[w], dp[w-item.weight]+value) Unbounded Knapsack: for each item: for w from item.weight→capacity: dp[w]=max(dp[w], dp[w-item.weight]+value)
