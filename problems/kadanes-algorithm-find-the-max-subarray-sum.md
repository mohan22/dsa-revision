# Problem: Kadane’s Algorithm find the max subarray sum
...
- Pattern hint from tags: DP / Arrays
- Use this note to reconstruct the full solution before promoting it to Tier 1.

## Core Insight

Iterate and keep `current = max(num, num + current)`, `best = max(best, current)`. Time O(n), Space O(1).
