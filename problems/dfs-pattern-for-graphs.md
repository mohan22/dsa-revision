# Problem: DFS pattern for graphs

## Source
- Platform: Anki deck seed
- Topic: Graph / Matrix
- Tags: Graph, DFS, Template, Day20
...
visited=set() def dfs(node): if node in visited: return; visited.add(node); for nei in graph[node]: dfs(nei) Used for components, cycle detection, clone graph.
