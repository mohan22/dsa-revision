# Problem: What are the DFS traversal patterns (preorder, inorder, postorder) used for
...
Preorder: build/copy trees. Inorder: BST logic (sorted order). Postorder: bottom-up problems. Template (postorder): left = dfs(node.left) right = dfs(node.right) process(left, right, node)
