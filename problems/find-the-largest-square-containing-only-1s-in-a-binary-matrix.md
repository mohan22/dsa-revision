...
- Tags: DynamicProgramming, Matrix, Day7
...
- Pattern hint from tags: DynamicProgramming / Matrix
- Use this note to reconstruct the full solution before promoting it to Tier 1.

## Core Insight

DP[i][j] = 1 + min(DP[i-1][j], DP[i][j-1], DP[i-1][j-1]) if matrix[i][j]=1. Track max.
