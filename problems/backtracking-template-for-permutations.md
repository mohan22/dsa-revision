# Problem: Backtracking Template For Permutations

## Source
- Platform: Anki deck seed
- Topic: Backtracking
- Tags: Backtracking, Permutations, Day24
...
What is the backtracking template for permutations


## Brief Problem Statement

Identify the core idea behind backtracking template for permutations and know when to apply it.
...
Use used[] array: if len(path)==n: add path. for i in range(n): if used[i]: continue; used[i]=True; path.append(nums[i]); backtrack(path); path.pop(); used[i]=False.
